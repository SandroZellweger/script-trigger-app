function generateDurationAlgoPinCitroenJumper(rowData, rowIndex, sheet) {
  // Check if a PIN is already present in column P (16th column, index 15)
  if (rowData[15]) {
    Logger.log("PIN already exists for row " + (rowIndex + 1));
    return;
  }
  ...
  // Place the generated PIN in the sheet in column P
  sheet.getRange(rowIndex, 16).setValue(pinData.pin);

  // Call the updateEventWithPIN function after generating the PIN
  updateEventWithPinCitroenJumper(rowData, pinData.pin);
}

function updateEventWithPinCitroenJumper(rowData, pin) {
  var vehicleType = 'N06 - Citroen Jumper (Minusio)'; // Ensure this matches your vehicleConfigs
  ...
  if (matchingEvent) {
    ...
    var newMessage = rentalDetails + locationDetails +
      ...
      "*Al Ritiro:*\n" +
      "- \uD83D\uDCF8 Spostare il veicolo in modo da poter fare le foto dei quatro lati in modo *FRONTALE* e mandare, prima della partenza\n" +
