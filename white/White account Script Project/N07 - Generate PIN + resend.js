function generateDurationAlgoPinRenaultTrafic(rowData, rowIndex, sheet) {
  ...
  var deviceId = 'IGK320803a7d'; // Renault Trafic device ID
  ...
  updateEventWithPinRenaultTrafic(rowData, pinData.pin);
}


function updateEventWithPinRenaultTrafic(rowData, pin) {
  var vehicleType = 'N07 - Renault Trafic (Lugano)'; // Ensure this matches your vehicleConfigs
  var vehicleConfig = vehicleConfigs[vehicleType];
  if (!vehicleConfig) {
    Logger.log("Vehicle Config not found for " + vehicleType);
    return; // Exit the function since we can't proceed without vehicleConfig
  }

  var calendarId = vehicleConfig.calendarId;
  var calendar = CalendarApp.getCalendarById(calendarId);

  var vehicleNumber = vehicleConfig.vehicleNumber;
  ...
  // Loop through events to find the one with the matching reference number
  for (var i = 0; i < events.length; i++) {
    var event = events[i];
    var description = event.getDescription() || '';

    // Check if the description contains the "Numero di riferimento"
    if (description.includes(numeroDiRiferimento)) {
      matchingEvent = event;
      break;
    }
  }
  ...
  if (matchingEvent) {
    ...
    var newMessage = rentalDetails + locationDetails +
      ...
      "- \uD83C\uDD7F\uFE0F Parcheggiare nel posteggio *No° 18*, rispettando le linee\n" +
      "- *NON è permesso posteggiare su altri posteggi*\n" +
    ...
    var whatsappLink =
      "https://wa.me/" +
      formatWhatsAppNumber(whatsappNumber.replace(/\D/g, '')) +
      "?text=" +
      encodedMessage;
...
// Make sure these match your existing IDs and variable names
var consolidatedSheetId = '1CfyZmrH0moAu8peDZfjtKu9eeGiWcXMTJ_c3eHgXm4o';

function rerunPinCreationForRow(rowIndex) {
  ...
  var vehicleType = rowData[18];
  
  if (!vehicleType) {
    Logger.log("No vehicle type found in row " + rowIndex);
    return;
  }
  
  var config = vehicleConfigs[vehicleType];
  if (!config) {
    Logger.log("No config found for vehicle type: " + vehicleType);
    return;
  }
  ...
  if (!igloPinFunctionName || typeof this[igloPinFunctionName] !== 'function') {
    Logger.log("No valid PIN generation function found for vehicle type: " + vehicleType);
    return;
  }
  
  try {
    this[igloPinFunctionName](rowData, rowIndex, sheet);
    Logger.log("PIN generation and event update re-run completed for row " + rowIndex);
  } catch (e) {
    Logger.log("Error re-running PIN creation for row " + rowIndex + ": " + e.toString());
  }
}
function runRerunForRow69() {
  rerunPinCreationForRow(344);
}